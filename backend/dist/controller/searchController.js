...
export default async function SearchController(req, res) {
    ...
    async function pagination() {
        ...
        try {
            if (title !== '' && keywords === '') {
                const [pages] = await db.execute(titleQuery, [finalizedTitle]);
                ...
                const [pages] = await db.execute(keywordQuery, [formatKeywords]);
                const numOfPages = Math.ceil(pages[0].total / limit);
                for (let i = 1; i <= numOfPages; i++) {
                    pagesArray.push(i);
                }
                ...
                const [pages] = await db.execute(titleAndKeywordQuery, [finalizedTitle, formatKeywords]);
                const numOfPages = Math.ceil(pages[0].total / limit);
                for (let i = 1; i <= numOfPages; i++) {
                    pagesArray.push(i);
                }
