...
export async function postReview(req, res) {
    ...
    async function ingestReview() {
        try {
            ...
            if (mediaType === 'movies') {
                await db.execute(`INSERT INTO reviews (userID, username, reviewScore, reviewText, movieID, showID, reviewID, dateTime) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [userID, username, reviewScore, reviewText, mediaID, null, reviewID, dateTime]);
            }
            // Logic for Show reviews: sets showID to the mediaID and movieID to null
            if (mediaType === 'shows') {
                await db.execute(`INSERT INTO reviews (userID, username, reviewScore, reviewText, movieID, showID, dateTime) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [userID, username, reviewScore, reviewText, null, mediaID, reviewID, dateTime]);
            }
...
export async function getReviews(req, res) {
    ...
    async function fetchReviews() {
        try {
            ...
            if (mediaType === 'movies') {
                const [totalResults] = await db.execute(`SELECT COUNT(*) AS total FROM reviews WHERE movieID = ?`, [mediaId]);
