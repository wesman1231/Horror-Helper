 ...
 *  }
 */
export default async function directorDataController(req, res) {
    /**
   ...
   */
    ...
    async function getDirectorImage() {
        try {
            const [directorImagePath] = await db.execute(`SELECT profilepath FROM directorinfo WHERE name = ?`, [formatDirectorName]);
    ...
    async function getDirectorProfile() {
        const [directorID] = await db.execute(`SELECT tmdbid FROM directorinfo WHERE name = ?`, [formatDirectorName]);
