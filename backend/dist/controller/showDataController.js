...
export async function fetchShowInfo(req, res) {
    //take the show id from the url, find that show in the database, and send it back to the client
    try {
        const showId = req.params.id;
        const [showInfo] = await db.execute('SELECT * FROM shows WHERE tmdbid = ?', [showId]);
...
export async function fetchNewReleases(req, res) {
    ...
    async function queryDB() {
        try {
            const [numberOfResults] = await db.execute(`SELECT COUNT(*) AS total FROM shows WHERE firstairdate LIKE ? AND firstairdate <= CURDATE();`, [`%${stringCurrentYear}%`]);
